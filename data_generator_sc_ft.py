...
class ShapeNetDataset(data.Dataset):
    def __init__(self, root_dir, train_type, transform=None):
        print(root_dir+'/img/')
        self.namelist = next(os.walk(root_dir+'/img/'))[2]
    ...
    def __getitem__(self, idx):
        im_path = self.root_dir+'/img/'+self.namelist[idx]
        img = cv2.imread(im_path)
        img = img.astype('float32')/255.0
        mask_path = self.root_dir+'/mask/'+self.namelist[idx]
        mask = cv2.imread(mask_path)
        mask = mask.astype('float32')/255.0
        mask = mask[:,:,0]
        label_path = self.root_dir+'/gt/'+self.namelist[idx]
