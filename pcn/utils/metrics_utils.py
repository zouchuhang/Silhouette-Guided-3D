...
def scale(gt_pc, pr_pc): #pr->[-0.5,0.5], gt->[-0.5,0.5]
	...
	min_gt = tf.convert_to_tensor([tf.reduce_min(gt[:,:,i], axis=1) for i in range(3)]) #(3, B)
	max_gt = tf.convert_to_tensor([tf.reduce_max(gt[:,:,i], axis=1) for i in range(3)]) #(3, B)
	min_pr = tf.convert_to_tensor([tf.reduce_min(pred[:,:,i], axis=1) for i in range(3)]) #(3, B)	
	max_pr = tf.convert_to_tensor([tf.reduce_max(pred[:,:,i], axis=1) for i in range(3)]) #(3, B)
	...
	new_min_gt = tf.convert_to_tensor([min_gt[i,:] - diff_gt[i,:]/2. for i in range(3)]) #(3, B)
	new_max_gt = tf.convert_to_tensor([max_gt[i,:] + diff_gt[i,:]/2. for i in range(3)]) #(3, B)
	new_min_pr = tf.convert_to_tensor([min_pr[i,:] - diff_pr[i,:]/2. for i in range(3)]) #(3, B)
	new_max_pr = tf.convert_to_tensor([max_pr[i,:] + diff_pr[i,:]/2. for i in range(3)]) #(3, B)
